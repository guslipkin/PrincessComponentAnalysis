...
```{r}
aniNrcWordLonger[, groupID := .GRP, by = list(n, movie)] %>%
  unique() %>%
  .[, !c("word")] %>%
  pivot_wider(names_from = sentiment, values_from = 1, values_fn = length) %>%
  ...
  geom_density_ridges(aes(x = startScale, y = movie, fill = movie), 
                      stat = "binline", color = "black", alpha = .9, bins = 50) +
  theme_ridges() +
...
aniAfinnWordLonger[, groupID := .GRP, by = list(n, movie)] %>%
  ...
  group_by(movie, song, value) %>%
  count(value) %>%
  ggplot() +
  geom_violin(aes(x = movie, y = value, fill = song), position = "dodge") + 
  ylim(-15, 15) +
  geom_hline(yintercept = 0, size = 1.5) +
  coord_flip() +
  labs(title = "Variation in positive and negative emotions by line")
...
aniAfinnWordLonger[, groupID := .GRP, by = list(n, movie)] %>%
  ...
  geom_hline(yintercept = 0, size = 1.5) +
  coord_flip() +
  labs(title = "Variation in positive and negative emotions by word")
```
