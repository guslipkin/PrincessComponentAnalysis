...
```{r}
# by line
tmp1 <- aniAfinnWordLonger[, groupID := .GRP, by = list(n, movie)] %>%
...
typeof(movieColors["Aladdin"])

# for some reason a data.table style mutate or select breaks this
tmp2 <- aniAfinnWordLonger %>%
...
movieData[aniNrcMovieWider, on = "movie"] %>%
  ggplot() +
  geom_line(aes(x = animatedRelease, y = totalFeeling), color = "purple", size = 2) +
  geom_line(aes(x = animatedRelease, y = totalSentiment), color = "blue", size = 2)
```
